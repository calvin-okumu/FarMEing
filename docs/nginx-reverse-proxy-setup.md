...
2. **Prepare Website Directory:**
   ...
   ```
   *Action:* Since your website is a Vite project, you must build it before deploying:
   1. On the server (or your local machine), run:
      ```bash
      cd ~/website
      npm install
      npm run build
      ```
   2. Copy the contents of the resulting `dist/` folder into `/var/www/farmtrack`:
      ```bash
      cp -r ~/website/dist/* /var/www/farmtrack/
      ```
